...
object Huffman {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs sortBy(pair => (pair._2, pair._1)) map(pair => Leaf(pair._1, pair._2))
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case one :: two :: rest => makeCodeTree(one, two) :: rest sortBy(tree => weight(tree))
      case _ => trees
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def treeLookup(subTree: CodeTree)(char: Char): List[Bit] = subTree match {
        case Leaf(_,_) => List()
        case Fork(l, r, _, _) => if (chars(l) contains char) 0 :: treeLookup(l)(char) else 1 :: treeLookup(r)(char)
      }
      text flatMap treeLookup(tree)
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table.find(code => code._1 == char).head._2
   ...
   */
   def appendBit(table: CodeTable, bit: Int): CodeTable = {
    table map (c => (c._1, bit :: c._2))
   }
    def convert(tree: CodeTree): CodeTable = tree match {
      case Leaf(c,_) => List((c,List()))
      case Fork(left,right,_,_) => mergeCodeTables(appendBit(convert(left), 0), appendBit(convert(right),1))
    }
