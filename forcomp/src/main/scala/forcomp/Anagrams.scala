...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase() groupBy(_.toChar) map(p => (p._1, p._2.length)) ).toList.sorted
  // w.toLowerCase() groupBy(_) flatMap((c,c))
  ...
  (occurrences foldRight List[Occurrences](Nil)) {
    case ((ch,occurrence), acc) => {
      acc ++ (for {acc1 <- acc; n <- 1 to occurrence} yield (ch,n)::acc1)
      }
    }
...
object Main extends App {
  ...
  val list = List("ate", "eat", "tea")
  println(sentenceOccurrences(list).toSet.subsets.map(_.toList).toSet )
